 ...
 */
tss_t tss_create_user_task(paddr_t code_start) { // recibe direccion fisica del comienzo del codigo de la tarea

  // se inicializa el directorio de páginas para la tarea
  // y se guarda el valor de cr3, que es el registro que apunta al directorio de páginas
  uint32_t cr3 = mmu_init_task_dir(code_start);

  // se establece el valor inicial del stack de la tarea
  // Se le resta 4KB a TASK_STACK_BASE para dejar espacio para la pila.
  vaddr_t stack = TASK_STACK_BASE - 4096;

  // se establece la dirección virtual donde comienza el código de la tarea.
  vaddr_t code_virt = TASK_CODE_VIRTUAL;

  // se solicita una página de kernel para la pila de nivel cero.
  // Cada tarea usa una pila distinta de nivel 0 (kernel).
  vaddr_t stack0 = mmu_next_free_kernel_page();

  // se establece el valor de esp0, que es el puntero de pila de nivel cero.
  // Debería apuntar al final de la página de la pila de nivel 0.
