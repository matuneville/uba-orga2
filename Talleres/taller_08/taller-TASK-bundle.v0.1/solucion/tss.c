...
// Devuelve un TSS Descriptor de nuestra GDT
...
tss_t tss_create_user_task(paddr_t code_start) {
  ...
  Dos tareas pueden tener una misma direccion virtual, pero como tienen diferente PDT (cr3), no hay problema, mapean a distinas direcciones fisicas.
  */

  vaddr_t code_virt = TASK_CODE_VIRTUAL;
  
  vaddr_t stack0 = mmu_next_free_kernel_page(); // pedir pagina de kernel para la pila de nivel cero

  vaddr_t esp0 = stack0 + PAGE_SIZE; // el stack arranca de abajo
