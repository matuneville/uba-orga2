...
/*
    Defino los segsel (segment selector) como GDT_CODE_0_SEL ya que la rutina de
    atencion a interrupcion es codigo que corre a nivel kernel.
    
    Luego, defino sel .dpl (descriptor privilege level) acorde al entry, que refiere
    al nivel de privilegio requerido para llamar a la interrupción (0: kernel, 3: user)

    El interrupt gate type lo definimos como 14,
    lo vemos en Table 3-2. System-Segment and Gate-Descriptor Types:
