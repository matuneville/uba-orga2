...
void mmu_map_page(uint32_t cr3, vaddr_t virt, paddr_t phy, uint32_t attrs) {
  ...
  if (!(PDT_Base[dir].attrs & MMU_P)){
    ...
    PDT_Base[dir].pt = new_PTAddress >> 12;
  }
...
paddr_t mmu_unmap_page(uint32_t cr3, vaddr_t virt) {
  ...
  if(PT_Base[table].attrs & MMU_P)
    PT_Base[table].attrs &= 0xFFE; // seteo bit P en 0  1111 1111 1110
...
paddr_t mmu_init_task_dir(paddr_t phy_start) {
  ...
  mmu_map_page(PDT_Base, TASK_STACK_BASE-PAGE_SIZE, pila_phy, MMU_P | MMU_U | MMU_W); 
