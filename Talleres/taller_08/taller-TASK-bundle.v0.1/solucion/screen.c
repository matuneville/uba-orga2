...
void screen_draw_box(uint32_t fInit, uint32_t cInit, uint32_t fSize,
  ...
  ca(*pantalla)[VIDEO_COLS] = (ca(*)[VIDEO_COLS])VIDEO;
  uint32_t row;
  uint32_t col;
  for (row = fInit; row < fInit + fSize; row++) {
    for (col = cInit; col < cInit + cSize; col++) {
      pantalla[row][col].c = character;
      pantalla[row][col].a = attr;
    }
  }
}
...
void screen_draw_layout(void) {
    ...
    uint32_t row;
    uint32_t col;
    for (row = 0; row < VIDEO_FILS; row++) {
      for (col = 0; col < VIDEO_COLS; col++) {
        pantalla[row][col].c = '*';
        pantalla[row][col].a = C_BG_CYAN;
      }
    }

      print("FURFI 2027", 20, 15, C_FG_BLACK + C_BG_CYAN);
}
